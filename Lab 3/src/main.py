...
async def leitor_bluetooth():
    while not fim:
        if luva_blt.any():
            ...
            #print(data)
        await asyncio.sleep(0)  # cede tempo para outras tarefas
...
async def modo3():
    ...
    pts = 0
    global fim
    funcoes_3 = [seta_cima, seta_baixo, seta_esquerda, seta_direita, desenha_z]
    ...
    while True:
        if tentativas == 10:
            break
        tentativas += 1
        desenho = urandom.choice(funcoes_3) #escolhe um desenho aleatório
        ...
        if desenho == seta_cima:
            ...
            if atual["y"] > y/2:# and abs(dx) < abs(dy):
                ...
                apagar()
                sleep(2)
        elif desenho == desenha_z:
            print("inverte")
            x, y, z = ref["invertido"]
            print(f"invertido = x:{x}, y:{y}, z:{z}")
            if atual["z"] < z/2:# and abs(dx) < abs(dy):
                certo()
                print("certo")
                apagar()
                pts += 1
...
# Configura UART0 nos pinos GP16 (TX) e GP17 (RX)
luva_blt = UART(0, baudrate=9600, tx=Pin(16), rx=Pin(17))
dados_recebidos = []
